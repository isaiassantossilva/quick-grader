...
import 'package:quick_grader/utils/input_image.dart';
import 'package:quick_grader/utils/utils.dart';
...
class CameraProcessing extends StatefulWidget {
  const CameraProcessing({super.key});

  @override
  State<CameraProcessing> createState() => _CameraProcessingState();
}

class _CameraProcessingState extends State<CameraProcessing>
      ...
      body: Column(
        children: <Widget>[
          ...
          Padding(
            ...
            child: Row(children: <Widget>[_cameraTogglesRowWidget()]),
          ),
        ],
      ),
    );
  ...
  Future<ui.Image> _rgbaBytesToImage(Uint8List data, int w, int h) async {
  ...
  void _processImage(CameraImage image) async {
    ...
    await cv.rectangleAsync(
      mat,
      cv.Rect(x, y, Random().nextInt(mat.width), Random().nextInt(mat.height)),
      cv.Scalar.red,
      thickness: 3,
    );
